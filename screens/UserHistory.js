...
const UserHistory = () => {
  ...
  getUserData().then(()=>{setRenderState(true);console.log(userData)}).catch((error)=> {setRenderState(false), console.log(error);})
  console.log('hi');
  }, [])
  ...
  let input_component = (field_name, field_value, key, visibility=false) => {
    return (
      <View style={styles.input_container}>
        ...
        <TextInput style={styles.inputBox} defaultValue={field_value} secureTextEntry={visibility}
          onChangeText={(text) => {
            if (field_value !== '') { 
  ...
  const update_particulars = async () => {
    if (passwordData['currentPassword'] === '') {
      setAlert({'status': true , 
                ...
                'message':'Please key in your current password to update your particulars', 
                'screen':'Profile'})
      ...
      setAlert({'status': true , 
                ...
                'screen':'Profile'})
    } else if (passwordData['newPassword'].length !== 0 && passwordData['newPassword'].length < 8) {
      setAlert({'status': true , 
                'title':'Unsuccessful Update', 
      ...
      if (passwordData['newPassword'].length !== 0) {
        await updatePassword(auth.currentUser, passwordData['password']).then(()=>{
                ...
                <KeyboardAwareScrollView>
                  ...
                  <View style={{width:'100%', alignItems:'center'}}>
                    <TouchableOpacity style={styles.submitButton} onPress={()=>{update_particulars()}}>
                      <Text style={{fontSize:17, fontWeight:'600'}}>update</Text>
                    </TouchableOpacity>
                  </View>
                </KeyboardAwareScrollView>
              </View>
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={{flex:4, justifyContent:'flex-end'}}>
          <TouchableOpacity style={{alignItems:'center',}}  onPress={()=>{navigation.navigate('MainInterface')}}>
        ...
        <AwesomeAlert show={alert['status']} 
          title={alert['title']} 
