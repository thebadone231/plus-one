...
const RequestScreen = () => {
  ...
  let input_component = (field_name, key, dynamic=false) => {
    return (
      <View style={styles.input_container}>
        ...
        <TextInput style={dynamic === true? {...styles.inputBox, height:heightDimension[key]}: styles.inputBox}
          multiline={true} 
          returnKeyType="done"
          ...
          onContentSizeChange={(Event)=>{//setHeightDimension(Event.nativeEvent.contentSize.height)
            if (dynamic === true) {
  ...
  const write_request = async () => {
    ...
    let location;
    await Geocoder.from(requestData['deliveryLocation']).then(
      json=>{
            ...
            <View style={{flex:6,}}>
              <KeyboardAwareScrollView>
                ...
                <View style={{alignItems:'center'}}>
                  <TouchableOpacity style={{backgroundColor:'white', justifyContent:'center', alignItems:'center', height:'27%', width:'26%', borderRadius:8, marginTop:40}}
