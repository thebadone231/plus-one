...
const ActivityScreen = () => {
  ...
  useEffect(() => {
    ...
    getUserData().then(() => {}).catch((error) => {console.error;});
    }, []);
  ...
  let Header = (section, index) => {
    return (
      <View style={{flexDirection:"row"}}>
        <View style={{backgroundColor:'#D5DDF9', flex:1}}><Text></Text></View>

        <View style={{flex:22, borderRadius: 10}}>
          <View style={{backgroundColor:'#D5DDF9'}}><Text></Text></View>
          ...
          <View style={{ backgroundColor: '#E8EDFF',  padding: 5, flexDirection:"row", borderRadius:3,}}>
            <View style={{flex:7, flexDirection:"row" ,justifyContent:"center"}}>
              <Text style={{ fontSize: 18, fontWeight: '600', marginRight:14, marginLeft:15 }}>{section.title["name"]}</Text>
              <Text style={{ fontSize: 18, fontWeight: '600',}}>{section.title["price"]}</Text>
            </View>

            <View style={{flex:2 }}>
              <Image style={{width:18, height:18 }} source={active_sections.includes(index) ? require('../../assets/circled-chevron-up.png'): require('../../assets/circled-chevron-down.png')}/>
            </View>
          </View>
        </View>
  ...
  let Content = (section) => {
    return (
      ...
      <View style={styles.content}>
        <Text style={{ fontSize: 13, fontWeight: '600', marginLeft: 20, marginBottom:7}}>{section.content["location"]}</Text>
        <View style={{flexDirection:"row", fontSize: 15, fontWeight: '400', marginLeft: 20, marginBottom:3, }}>
          <Text>Order details: </Text>
          <Text style={{width:"74%",}}>{section.content["details"]["order specifics"]}</Text>
        </View>
        <Text style={{ fontSize: 15, fontWeight: '400', marginLeft: 20, marginBottom:3}}>Time of request  : {section.content["time"]}</Text>
        <Text style={{ fontSize: 15, fontWeight: '400', marginLeft: 20, marginBottom:3}}>Payment method: {section.content["details"]["payment method"]}</Text>
        <Text style={{ fontSize: 15, fontWeight: '400', marginLeft: 20, marginBottom:8}}>Delivered by: {section.content["details"]["delivered by"]["user"]}</Text>
      </View>
  ...
  if (time && type) {
    ...
    let dateFilter = (ref, item) => {
      var curr = (ref.getFullYear()-1) * 365 + ref.getMonth()*31 + ref.getDate();
      var prev = (parseInt(item.split(" ")[0].split("-")[0])-1)*365 + (parseInt(item.split(" ")[0].split("-")[1])-1)*31 + parseInt(item.split(" ")[0].split("-")[2])
      return curr <= prev
    }
    ...
    if (sections.length!=0) {
      ...
      main_component = 
      <View style={{alignItems:"center"}}>
        <Text>Information on the fields selected are missing {"\n"}                 Please select other fields</Text>
      </View>
    ...
    <View style={styles.container}>
      ...
      <View style={{ flex: 23 }}>
        <View style={{flex: 1, flexDirection: 'row', justifyContent: 'space-evenly',paddingTop: 20,}}>
          ...
          <View style={{}}>
            <SelectDropdown data={field2} 
              buttonTextAfterSelection={(selectedItem) => {return selectedItem['label'];}}
              onSelect={(selectedItem) => {sections = []; setTime(selectedItem['value']);}}
              rowTextForSelection={(item, index) => {return item['label'];}}
              buttonStyle={{ width: 140, height: '50%', borderRadius: 6 }}
              buttonTextStyle={{ fontSize: 15, fontWeight: '600' }}
              rowStyle={{ height: 33 }}
        ...
        <View style={{ flex: 8 }}>
          <View contentContainerStyle={{ alignItems: 'center', height: 200, width: 350, borderRadius: 15, padding: 10  }}>
          {main_component}
          </View>
        </View>
